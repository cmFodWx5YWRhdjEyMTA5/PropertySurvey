...
public class AppAuthInterceptor extends AppBaseInterceptor {
    ...
    private static final int ERROR_UNAUTORIZATION = 401;
    ...
    public AppAuthInterceptor(@ApplicationContext Context context,
                              AppSessionManager userSession, ILoginInterceptor loginInterceptor) {
    ...
    protected Map<String, String> getHeaderMap(
            String path, String strParam, String method, String contentTypeHeader) {

        Map<String, String> headerMap = new ArrayMap<>();
        if(userSession.getSession() != null) {
            this.authKey = userSession.getSession().getApi_key();
        }
        if(this.authKey == null) {
            loginInterceptor.startLogin();
            return headerMap;
        }
    ...
    protected Response checkForceLogout(Interceptor.Chain chain, Response response, Request finalRequest) throws
            IOException {
        if(response.code() == ERROR_UNAUTORIZATION) {
