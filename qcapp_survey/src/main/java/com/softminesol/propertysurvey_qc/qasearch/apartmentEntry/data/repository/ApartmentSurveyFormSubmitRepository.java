package com.softminesol.propertysurvey_qc.qasearch.apartmentEntry.data.repository;
...
public class ApartmentSurveyFormSubmitRepository implements IApartmentSurveyFormSaveRepository {
    ...
    @Override
    public Observable<GetPropertySaveResponse> submitCloudNewApartment(String id,final SaveApartmentRequest formData) {
        ...
        if(formData.getApartmentImagepath().size()>0) {
            ...
            return submitFormDataFactory.getCloudSubmitFomData().submitCloudNewApartment(id,formData).doOnError(new Action1<Throwable>() {
                @Override
