package com.softminesol.propertysurvey_qc.qasearch.apartmentEntry.data.repository.datasource;

import com.softminesol.propertysurvey_qc.qasearch.apartmentEntry.data.net.NewServeyAPI;
...
public class CloudSubmitFomData {
    ...
    public Observable<GetPropertySaveResponse> submitCloudNewApartment(String id, SaveApartmentRequest formData) {
        return newServeyAPI.submitNewApartment(id,formData).map(new Func1<Response<DataResponse<GetPropertySaveResponse>>, GetPropertySaveResponse>() {
            @Override
